...
//    self.topView.backgroundColor = [UIColor colorWithWhite:0.4 alpha:0.4];
    ...
    self.bottomView.image = [UIImage imageNamed:@"community_video_layer"];
...
//    self.playOrPauseBtn.showsTouchWhenHighlighted = YES;
    ...
    [self.playOrPauseBtn setImage:[UIImage imageNamed:@"community_video_play"]forState:UIControlStateNormal];
    [self.playOrPauseBtn setImage:[UIImage imageNamed:@"community_video_off"]forState:UIControlStateSelected];

    [self.bottomView addSubview:self.playOrPauseBtn];
    //autoLayout _playOrPauseBtn
    [self.playOrPauseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.bottomView).with.offset(1);
        make.height.mas_equalTo(50);
    ...
    [self.progressSlider setThumbImage:[UIImage imageNamed:@"community_video_again_circular"] forState:UIControlStateNormal];
    self.progressSlider.minimumTrackTintColor = [UIColor whiteColor];
    self.progressSlider.maximumTrackTintColor = COLOR_2;
    ...
    [self.progressSlider mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.bottomView).with.offset(ZH_SCALE_SCREEN_Width(77.5));
        make.right.equalTo(self.bottomView).with.offset(-ZH_SCALE_SCREEN_Width(77.5));
...
//    self.fullScreenBtn.showsTouchWhenHighlighted = YES;
    ...
    [self.fullScreenBtn setImage:[UIImage imageNamed:@"community_video_enlarge"] forState:UIControlStateNormal];
    [self.fullScreenBtn setImage:[UIImage imageNamed:@"community_video_enlarge"]forState:UIControlStateSelected];
    [self.bottomView addSubview:self.fullScreenBtn];
    //autoLayout fullScreenBtn
    [self.fullScreenBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.bottomView).with.offset(-1);
        make.height.mas_equalTo(50);
    ...
    self.leftTimeLabel.font = [UIFont systemFontOfSize:ZH_SCALE_SCREEN_Height(12)];
    [self.bottomView addSubview:self.leftTimeLabel];
    //autoLayout timeLabel
    [self.leftTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.bottomView).with.offset(ZH_SCALE_SCREEN_Width(35));
        make.right.equalTo(self.bottomView).with.offset(ZH_SCALE_SCREEN_Width(-35));
        make.height.mas_equalTo(50);
        make.bottom.equalTo(self.bottomView).with.offset(ZH_SCALE_SCREEN_Height(0));
    }];
    ...
    self.rightTimeLabel.font = [UIFont systemFontOfSize:ZH_SCALE_SCREEN_Height(12)];
...
#pragma mark
#pragma mark lazy 加载失败的btn
-(UIButton *)loadFailedBtn{
    if (_loadFailedBtn==nil) {
        _loadFailedBtn = [[UIButton alloc]init];
        _loadFailedBtn.hidden = YES;
        [_loadFailedBtn addTarget:self action:@selector(loadFailedBtnClick) forControlEvents:UIControlEventTouchUpInside];
        [_loadFailedBtn setImage:[UIImage imageNamed:@"community_video_again_load"] forState:UIControlStateNormal];
        [self.contentView addSubview:_loadFailedBtn];
        [_loadFailedBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.center.equalTo(self.contentView);
            make.size.mas_equalTo(CGSizeMake(ZH_SCALE_SCREEN_Width(108), ZH_SCALE_SCREEN_Height(67)));
...
#pragma mark  -- 加载失败按钮的点击方法 点击从新加载
- (void)loadFailedBtnClick{
    [self play];
}
#pragma mark
#pragma mark 进入后台
...
-(void)setTitleStr:(NSString *)titleStr{
    ...
    // 计算字符串宽度
    CGFloat width=[(NSString *)_titleStr boundingRectWithSize:CGSizeMake(MAXFLOAT, 30) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:SCREEN_HEIGHT/667 *14]} context:nil].size.width;

    // 小于屏幕宽度就label不动, 大于就走跑马灯效果
    if (width <= SCREEN_WIDTH/375 *320) {
        self.titleLabel = [[UILabel alloc]init];
        self.titleLabel.text =_titleStr;
        self.titleLabel.textColor = COLOR_8;
        self.titleLabel.numberOfLines = 1;
        self.titleLabel.font = [UIFont systemFontOfSize:SCREEN_HEIGHT/667 *14];
        self.titleLabel.textAlignment = NSTextAlignmentCenter;
        self.titleLabel.backgroundColor = [UIColor clearColor];
        [self.topView addSubview:self.titleLabel];
        
        [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.topView).with.offset(37);
            make.right.equalTo(self.topView).with.offset(-12);
            make.center.equalTo(self.topView);
            make.top.equalTo(self.topView).with.offset(3);
            
        }];
        
    }else{
        LSPaoMaView* paomav = [[LSPaoMaView alloc] initWithFrame:CGRectMake(0, 200, SCREEN_WIDTH, SCREEN_HEIGHT/667*30) title:_titleStr];
        paomav.backgroundColor = [UIColor clearColor];
        paomav.textLb.textColor = COLOR_8;
//        paomav.textLb.backgroundColor = COLOR_9;
        paomav.textLb.font = [UIFont systemFontOfSize:SCREEN_HEIGHT/667 *14];
        paomav.textLb.textAlignment= NSTextAlignmentCenter;
        paomav.reserveTextLb.textColor = COLOR_8;
//        paomav.reserveTextLb.backgroundColor = COLOR_15;
        paomav.reserveTextLb.font = [UIFont systemFontOfSize:SCREEN_HEIGHT/667 *14];
        paomav.reserveTextLb.textAlignment= NSTextAlignmentCenter;
        [self.topView addSubview:paomav];
        [paomav mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.topView).with.offset(37);
            make.right.equalTo(self.topView).with.offset(-12);
            make.center.equalTo(self.topView);
            make.top.equalTo(self.topView).with.offset(19);
...
//                    }
                ...
                case AVPlayerStatusFailed:
                {
                    ...
                    if (error) {
                        ...
                        [self bringSubviewToFront:self.loadFailedBtn];
                        //here
                        [self.loadingView stopAnimating];
                    }
...
-(void)hideTheLightViewWithHidden:(BOOL)hidden{
    if (self.isFullscreen) {//全屏才出亮度调节的view
        ...
        if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
            self.effectView.frame = CGRectMake(SCREEN_HEIGHT/2-155/2, SCREEN_WIDTH/2-155/2, 155, 155);
        }
    }else{
        return;
    }
