...
/** 搜索不可变数组 */
@property (nonatomic, strong) NSArray *searchHistorieArrs;
...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        ...
        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT - 64) style:UITableViewStyleGrouped];
...
- (PYSearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        if (self.searchHistoryStyle == PYSearchHistoryStyleNil) {
            searchSuggestionVC.wolfKill = @"wolfKill";
        }
...
- (NSArray *)searchHistorieArrs
{
    
    if (!_searchHistorieArrs) {
        _searchHistorieArrs = [NSKeyedUnarchiver unarchiveObjectWithFile:self.searchHistoriesCachePath];
        if (!_searchHistorieArrs) {
            _searchHistorieArrs = [NSArray array];
        }
    }
    return _searchHistorieArrs;
...
- (void)setup
{
    ...
    lineView.backgroundColor = COLOR_16;
    lineView.frame = CGRectMake(0, 64 - 0.5, SCREEN_WIDTH, 0.5);
    ...
    //右边按钮
    CGSize size = [UIlabelTool sizeWithString:@"取消" font:[UIFont systemFontOfSize:15] width:SCREEN_WIDTH];
    myButton *RightBtn = [myButton buttonWithType:UIButtonTypeCustom frame:CGRectMake(SCREEN_WIDTH - SCREEN_WIDTH/375*24 - size.width, 20, SCREEN_WIDTH/375*24 + size.width, 44) title:@"取消" titleColor:COLOR_3 backgroundImage:nil andBlock:^{
    ...
    titleView.py_y = 27;
    titleView.py_width = self.view.py_width - 28 - titleView.py_x * 2;
    titleView.py_height = WCX_SCALE_SCREEN_Height(30);
    ...
    hotSearchTagsContentView.py_y = CGRectGetMaxY(titleLabel.frame) + WCX_SCALE_SCREEN_Height(25);
   ...
   #pragma mark  -- 底部清除历史搜索
    // 设置底部(清除历史搜索)
    UIView *footerView = [[UIView alloc] init];
    footerView.py_width = PYScreenW;
    
    UIButton *emptySearchHistoryBtn =[UIButton buttonWithType:UIButtonTypeCustom];
    [emptySearchHistoryBtn setTitleColor:COLOR_3 forState:UIControlStateNormal];
    emptySearchHistoryBtn.titleLabel.font =[UIFont systemFontOfSize:WCX_SCALE_SCREEN_Height(14)];
    [emptySearchHistoryBtn setTitle:PYEmptySearchHistoryText forState:UIControlStateNormal];
    [emptySearchHistoryBtn setImage:[UIImage imageNamed:@"address_delete"] forState:UIControlStateNormal];
    [emptySearchHistoryBtn addTarget:self action:@selector(emptySearchHistoryDidClick) forControlEvents:UIControlEventTouchUpInside];
    emptySearchHistoryBtn.py_height = 30;
    emptySearchHistoryBtn.py_width = PYScreenW;

    [footerView addSubview:emptySearchHistoryBtn];
...
- (UILabel *)setupTitleLabel:(NSString *)title
{
    ...
    titleLabel.font = [UIFont systemFontOfSize:WCX_SCALE_SCREEN_Height(14)];
    titleLabel.tag = 1;
    [titleLabel sizeToFit];
    titleLabel.py_x = 0;
    titleLabel.py_y = SCREEN_HEIGHT/667*22;
    return titleLabel;
}
...
- (void)setSearchHistoriesCachePath:(NSString *)searchHistoriesCachePath
{
    ...
    }  else if (self.searchHistoryStyle == PYSearchHistoryStyleNil){
        return;
    } else { // 搜索历史为标签类型
...
- (void)setSearchText:(NSString *)searchText
{
    ...
    // 赋值给搜索建议控制器
    self.searchSuggestionVC.searchText = searchText;
}
...
- (void)setSearchHistoryStyle:(PYSearchHistoryStyle)searchHistoryStyle
{
    ...
    if (searchHistoryStyle == PYSearchHistoryStyleNil) {
        self.baseSearchTableView.tableFooterView = nil;
        return;
    }
    // 默认cell，直接返回
...
- (UILabel *)labelWithTitle:(NSString *)title
{
    ...
    label.font = [UIFont systemFontOfSize:WCX_SCALE_SCREEN_Height(12)];
    label.text = title;
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    if (self.searchHistories.count > 6) {
        self.searchHistories = [NSMutableArray arrayWithArray:[self.searchHistories subarrayWithRange:NSMakeRange(0, 6)]];
    }
...
- (void)closeDidClick:(UIButton *)sender
{
    ...
    ZHLog(@"%@ ----- %@",cell.textLabel.text,self.searchHistories);
...
//        cell.textLabel.font = [UIFont systemFontOfSize:14];
        ...
        // 设置图片容器大小、图片原图居中
        closetButton.frame = CGRectMake(SCREEN_WIDTH - cell.py_height, 0, cell.py_height, cell.py_height);
        [closetButton setImage:[UIImage imageNamed:@"sayu_close"] forState:UIControlStateNormal];
        [closetButton addTarget:self action:@selector(closeDidClick:) forControlEvents:UIControlEventTouchUpInside];
        [cell.contentView addSubview:closetButton];
    ...
    cell.textLabel.font = [UIFont systemFontOfSize:WCX_SCALE_SCREEN_Height(14)];
...
{
    return SCREEN_HEIGHT/667*30;
}

#pragma mark  -- 设置“历史搜索”的大小以及颜色
-(void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section
{
    view.tintColor = [UIColor whiteColor];
    UITableViewHeaderFooterView *header = (UITableViewHeaderFooterView *)view;
    header.contentView.backgroundColor = [UIColor clearColor];
    [header.textLabel setTextColor:COLOR_3];
    [header.textLabel setFont:[UIFont systemFontOfSize:WCX_SCALE_SCREEN_Height(14)]];
}
