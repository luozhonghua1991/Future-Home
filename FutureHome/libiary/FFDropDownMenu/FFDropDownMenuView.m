...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.menuAnimateType = FFDropDownMenuViewAnimateType_ScaleBasedTopLeft;
...
//    self.triangleView.frame = CGRectMake(horizonWidth - self.triangleRightMargin - self.triangleSize.width, self.realTriangleY + 5, self.triangleSize.width, self.triangleSize.height);
    self.triangleView.frame = CGRectMake(self.triangleRightMargin, self.realTriangleY, self.triangleSize.width, self.triangleSize.height);
    self.triangleView.hidden = YES;
    self.triangleView.triangleColor = self.triangleColor;
    
    
    
    NSLog(@"menuWidth的宽度%f\nmenuRightMargin的宽度%f",self.menuWidth,self.menuRightMargin);
    
    
    //tableView的frame <set tableView frame>
//    self.menuViewFrame = CGRectMake(horizonWidth - self.menuWidth - self.menuRightMargin, CGRectGetMaxY(self.triangleView.frame) + 5, self.menuWidth, self.eachMenuItemHeight * self.menuModelsArray.count);
    self.menuViewFrame = CGRectMake(self.menuRightMargin, 86, self.menuWidth, self.eachMenuItemHeight * self.menuModelsArray.count);
