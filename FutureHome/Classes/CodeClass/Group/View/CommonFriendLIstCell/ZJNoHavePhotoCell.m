...
-(void)setModel:(ZJCommit *)model {
    ...
    self.bottomView.frame = CGRectMake(0, MaxY(self.contentLab) + 10, SCREEN_WIDTH, 42);
...
- (void)upBtnClick {
    if (_delegate != nil && [_delegate respondsToSelector:@selector(fh_ZJNoHavePhotoCellSelecLiketModel:withBtn:)]) {
        [_delegate fh_ZJNoHavePhotoCellSelecLiketModel:self.model withBtn:self.upBtn];
    }
}
