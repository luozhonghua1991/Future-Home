...
static CGFloat const TITLEBUTTONWIDTH = 65.0;
...
- (UIPickerView *)addressPickerView{
    if (!_addressPickerView) {
        _addressPickerView = [[UIPickerView alloc]initWithFrame:
                              CGRectMake(0, TITLEHEIGHT, SELFSIZE.width, 220)];
        _addressPickerView.backgroundColor = [UIColor colorWithRed:239/255.f
                                                             green:239/255.f
                                                              blue:244.0/255.f
                                                             alpha:1.0];
//        _addressPickerView.backgroundColor = [UIColor redColor];
...
- (void)showOrHide:(BOOL)isShow{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (isShow) {
            selfkY = [UIScreen mainScreen].bounds.size.height - 260;
        }
        else {
            selfkY = [UIScreen mainScreen].bounds.size.height;
        }
        self.frame = CGRectMake(0,selfkY, self.bounds.size.width,260);
